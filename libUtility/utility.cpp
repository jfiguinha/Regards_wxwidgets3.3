...
void Error::CheckError(cl_int err)
{
	if (err != CL_SUCCESS)
	{
		printf("Error : %s happened in file %s at line %s /n", CConvertUtility::ConvertToStdString(opencl_error_to_str(err)),
		       CConvertUtility::ConvertToStdString(to_str(__FILE__)), CConvertUtility::ConvertToStdString(to_str(__LINE__)));
