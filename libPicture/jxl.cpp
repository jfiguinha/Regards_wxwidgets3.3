 ...
 */
bool CJxl::DecodeJpegXlOneShot(const uint8_t* jxl,
    size_t size,
    cv::Mat& matFloat,
    size_t& xsize,
    size_t& ysize,
...
{
    if (jxl == nullptr || size == 0 || icc_profile == nullptr)
        return false;
    ...
    for (;;)
    {
        ...
        else if (status == JXL_DEC_NEED_IMAGE_OUT_BUFFER)
        {
            ...
            const size_t expected_size =
                xsize * ysize * 4 * sizeof(float);

            if (buffer_size != expected_size)
            {
                fprintf(stderr,
                    "Invalid out buffer size %zu %zu\n",
                    buffer_size,
                    expected_size);
                return false;
            }
            ...
            const size_t pixels_buffer_size =
                matFloat.total() * matFloat.elemSize();
    ...
    std::vector<uint8_t>* compressed)
{
    ...
    {
        fprintf(stderr,
            "Invalid JPEG XL input format: expected CV_32FC4\n");
        return false;
    }
    ...
    {
        fprintf(stderr,
            "JxlThreadParallelRunnerMake failed\n");
        return false;
    }
    ...
    basic_info.xsize =
        static_cast<uint32_t>(matFloat.cols);

    basic_info.ysize =
        static_cast<uint32_t>(matFloat.rows);
    ...
    const wxString& path)
{
    ...
    CPictureUtility::writefile(
        path,
        compressed.data(),
        compressed.size());
}

void CJxl::GetDimensions(
    const wxString& jxl_filename,
    int& width,
    int& height)
{
    width = 0;
    ...
    auto info =
        static_cast<JxlBasicInfo*>(
            DecodeJpegDim(file));

    if (info == nullptr)
    {
        fprintf(stderr,
            "Couldn't read basic info\n");
    ...
    const wxString& path)
{
    ...
    std::vector<uint8_t> compressedImage =
        CPictureUtility::ReadFile(path);

    if (compressedImage.empty())
        return cv::Mat();

    if (!DecodeJpegXlOneShot(
