...
cv::Mat CRegardsWebp::GetPicture(const wxString& filename)
{
    std::vector<uint8_t> compressedImage =
        CPictureUtility::ReadFile(filename);
...
std::vector<cv::Mat> CRegardsWebp::GetAllPicture(
    ...
    int& delay)
{
        ...
        &anim_info))
    {
        fprintf(
            stderr,
            "Error getting global info about the animation\n");
    ...
    while (WebPAnimDecoderHasMoreFrames(dec))
    {
            ...
            &timestamp))
        {
            fprintf(
                stderr,
                "Error decoding WebP animation frame\n");
        ...
        const size_t frameSize =
            static_cast<size_t>(anim_info.canvas_width) *
            static_cast<size_t>(anim_info.canvas_height) *
            4;
...
void CRegardsWebp::GetPictureDimension(
    const wxString& filename,
    int& width,
    int& height)
{
